class Chunk {
	...
	constructor( x, z, callback ) {
		...
		this.terrainHillRange = gridSizeY * 0.6;
	...
	initGrid() {
		...
		for ( var z = 0; z < gridSize; z ++ ) {
			...
			for ( var x = 0; x < gridSize; x ++ ) {
				...
				//low frequency noise
				let lowN = noise(
					( x + this.offset.x * ( gridSize - 1 ) - this.offset.x ) * this.noiseScale,
					( z + this.offset.z * ( gridSize - 1 ) - this.offset.z ) * this.noiseScale,
				);

				//high frequency noise
				let highN = noise(
					( x + this.offset.x * ( gridSize - 1 ) - this.offset.x ) * this.noiseScale * 4,
					( z + this.offset.z * ( gridSize - 1 ) - this.offset.z ) * this.noiseScale * 4,
				);
				...
				for ( var y = 0; y < terrainHeight; y ++ ) {
					...
					//set all gridcells to solid with 0.5. ( < 0 is empty )
					this.setValueToGrid( x, y, z, 0.5 );
	...
	generateMesh() {
		...
		return new Promise( resolve =>{
			...
			surfaceNetEngine.createSurface( this.grid, [ gridSize, gridSizeY, gridSize ] ).then( generatedSurface =>{
				...
				generatedSurface.faces.forEach( ( f, fIndex )=>{
					...
					//generate uv's for texturing later on
				...
				//add an edgesgeometry to the mesh with the preloaded linematerial
				let edgesgeo = new THREE.EdgesGeometry( geo );
				let edges = new THREE.LineSegments( edgesgeo, lineMaterial );
				this.terrainMesh.add( edges );
