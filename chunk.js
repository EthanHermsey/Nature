class Chunk {
	...
	constructor( x, z, parent, callback ) {
		...
		this.terrainPlaneMesh.material.color.setRGB( 0.5, 0.5, 0.5 );
	...
	showLevel( level ) {
		...
		// if ( this.offset.x == 1 && this.offset.z == - 16 ) {

		// 	console.log( 'set lod to: ', level );
		// 	console.log( player.currentChunkCoord );
		// 	console.log( this );
		...
		if ( this.lodLevel == 1 ) {
			...
			// if ( this.parent.castChunks.includes( this.terrainMesh ) ) {

			// this.terrainPlaneMesh.material.color.set( 'red' );
			...
			// this.terrainPlaneMesh.material.color.set( 'green' );
	...
	initGrid() {
		...
		for ( var z = 0; z < this.gridSize.z; z ++ ) {
			...
			for ( var x = 0; x < this.gridSize.x; x ++ ) {
				...
				//terracing
				// terrainHeight = Math.round( terrainHeight / 2 ) * 2;
				...
				for ( var y = 0; y < terrainHeight + smoothRange; y ++ ) {
					...
					// //extra ridge for watter?
					// if ( y < this.gridSize.y * 0.2 && y > 0 ) v += 0.5 * pow( noise(
					// 	5999755664 + ( x + this.offset.x * ( this.gridSize.x - 1 ) - this.offset.x ) * this.noiseScale * 0.10,
					// 	5999755664 + ( z + this.offset.z * ( this.gridSize.z - 1 ) - this.offset.z ) * this.noiseScale * 0.10,
					// ), 2 );
	...
	generateCliffs() {
		...
		if ( cliffGeo ) {
			...
			// this.cliff = new THREE.Mesh( cliffGeo, this.parent.cliffMaterial );
