...
class Player {
	...
	constructor( startChunk ) {
		...
		this.skyBox = new THREE.Mesh(
			...
			new THREE.MeshBasicMaterial( {
				map: new THREE.TextureLoader().load( './resources/background1.jpg' ),
				side: THREE.BackSide
			} )
		);
		...
		// const lensFlare0 = new THREE.TextureLoader().load( './resources/lensflare0.png' );
		// const lensFlare1 = new THREE.TextureLoader().load( './resources/lensflare1.png' );

		// this.lensFlare = new THREE.Lensflare();
		// this.lensFlare.addElement( new THREE.LensflareElement( lensFlare1, 105, 0.4 ) );
		// this.lensFlare.addElement( new THREE.LensflareElement( lensFlare1, 200, 0.6 ) );
		// this.lensFlare.addElement( new THREE.LensflareElement( lensFlare1, 500, 0.9 ) );
		// this.lensFlare.addElement( new THREE.LensflareElement( lensFlare0, 800, 0, new THREE.Color( 'yellow' ) ) );

		// this.lensFlare.material.opacity = 1.0;
		// this.lensFlare.material.transparent = false;
		// this.shadowLight.add( this.lensFlare );
	...
	update( delta ) {
		...
		if ( intersectDown[ 0 ] ) {
			...
			this.currentChunkCoord = this.getChunkCoord( this.position, chunk.chunkSize );
