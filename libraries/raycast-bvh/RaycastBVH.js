	...
	return function intersectsTriangle( other, target = null ) {
		...
		if ( target ) {
			...
			if ( Math.abs( plane1.normal.dot( plane2.normal ) ) > 1.0 - 1e-10 ) {
				...
				console.warn( 'SeparatingAxisTriangle.intersectsTriangle: THREE.Triangles are coplanar which does not support an output edge. Setting edge to 0, 0, 0.' );
...
// Ripped and modified From THREE.js THREE.Mesh raycast
...
[
	...
	MeshBVH.prototype[ name ] = function ( ...args ) {
		...
		if ( args[ 0 ] === null || args[ 0 ].isMesh ) {
			...
			args.shift();
			console.warn( `MeshBVH: The function signature for "${ name }" has changed and no longer takes THREE.Mesh. See docs for new signature.` );
