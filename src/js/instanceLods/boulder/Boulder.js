...
export default class Boulder extends CachedMesh {
	...
	removeMatricesOnDistanceFromPoint( chunkKey, point, distance ) {
		...
		const checkData = ( array ) => {
			...
			return array.filter( data => {
				...
				p.copy( data.boundingSphere.center ).multiply( this.terrain.terrainScale ); //custom
	...
	generateMesh( chunkKey ) {
		...
		if ( data.geometries.length > 0 ) {
			...
			const newGeometry = BufferGeometryUtils.mergeBufferGeometries( data.geometries, true );
	...
	generateData( chunk ) {
		...
		let dummy = new THREE.Object3D(), up = new THREE.Vector3( 0, 1, 0 );
		let geometries = [];
		for ( let i = 0; i < chunked.length; i ++ ) {
